    ...
    QString* errorMessage)
{
    ...
    stream << "Segment,From Point,To Point,Chainage Start (m),Chainage End (m),Horizontal Distance (m),3D Distance (m),Delta Z (m),Below Threshold,Start X,Start Y,Start Z,End X,End Y,End Z\n";
    ...
    for (int segmentIndex = 0; segmentIndex < analysisResult.segments.size(); ++segmentIndex) {
        ...
        stream
            ...
            << csvCell(segment.belowThreshold ? QStringLiteral("Yes") : QStringLiteral("No")) << ','

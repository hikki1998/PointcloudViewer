...
### 会话目标
- 调研同类点云巡检/处理软件核心能力。
- 结合本项目现状，形成“可迁移性优先”的新功能开发计划。

### 已完成
- 读取仓库现状文档，确认当前产品能力边界。
- 抓取并整理竞品公开能力信息：DJI Terra、Trimble RealWorks、TerraScan、Pix4Dmatic、iTwin Capture、CloudCompare。
- 将竞品能力归一到统一能力域并映射到本项目差距。
- 输出分阶段的新功能路线图草案（近期/中期/远期）与优先级建议。

### 输出重点
- 优先建议从“工作流批处理、变化检测、报告模板化、插件化接口、开放交换层”五类能力切入。
- 所有建议均强调跨行业和跨平台迁移价值，尽量避免绑死单一硬件生态。

...
- 多个 `.las/.laz` 数据加载与项目树管理；单文件、多文件和追加导入均支持后台读取和 CPU 渲染几何准备，并提供进度/取消；大文件单文件打开时先显示 preview，多数据集使用独立渲染节点
...
- 大数据集交互时使用有上限的抽样节点；鼠标拖动按住期间保持预览，释放并静止后恢复完整节点，避免交互 LOD 来回切换闪烁
- 视角预设、场景适配、悬停坐标显示
- RGB / 高程 / 单色 / 分类显示

...
### LAS/LAZ 加载与渲染
1. 用户打开、追加或拖放的 LAS/LAZ 由 `PointCloudViewer` 工作线程调用 `LasReader` 后台读取，并在同一工作线程构建相对原点的 float OSG 顶点/颜色数组；UI 线程只提交已准备场景节点，支持多文件串行进度和取消；大文件单文件打开时先显示有上限的 preview，再原子替换为完整数据；工程恢复保留同步兼容路径
...
5. 超过阈值的数据集额外保留一个最多约 18 万点的交互预览节点；相机运动时通过 `NodeMask` 显示预览，鼠标按住期间不恢复，释放并静止约 150ms 后显示完整节点，不复制全量点云
